...
<script runat="server">
    ...
    Public Overrides Function Validate() As Boolean
        ...
        '    errors.Text = "<div class=""error sheen"">" & error_html & "</div><br />"
        '    Return False
        'End If
        'Return True
    End Function

    Public Overrides Sub DoPayment(ByVal order As IOrder, ByVal paymentSuccessCallback As Action(Of ITransaction))
        'Dim transaction As New SagePayDirectTransaction()
        'transaction.OrderID = order.ID
        'Dim helper As New SagePayTransactionHelper
        'helper.Add("TxType", "PAYMENT")
        ...
        'helper.Add("Amount", Decimal.Parse(FormatNumber(_Basket.TotalGrossPrice.Amount, 2)))
        ...
        'helper.Add("BillingFirstnames", _Basket.BillingAddress.Name.ForeName)
        'helper.Add("BillingSurname", _Basket.BillingAddress.Name.Surname)
        'helper.Add("BillingAddress1", _Basket.BillingAddress.Line1)
        'helper.Add("BillingAddress2", _Basket.BillingAddress.Line2)
        'helper.Add("BillingAddress3", _Basket.BillingAddress.Line3)
        'helper.Add("BillingCity", _Basket.BillingAddress.Line4)
        'helper.Add("BillingPostCode", _Basket.BillingAddress.PostCode)
        'helper.Add("BillingCountry", _Basket.BillingAddress.Country.Code)
		
        'If _Basket.BillingAddress.Country.HasStates Then
        '    helper.Add("BillingState", _Basket.BillingAddress.State.Code)
        'End If
		
        'helper.Add("DeliveryFirstnames", _Basket.DeliveryAddress.Name.ForeName)
        'helper.Add("DeliverySurname", _Basket.DeliveryAddress.Name.Surname)
        'helper.Add("DeliveryAddress1", _Basket.DeliveryAddress.Line1)
        'helper.Add("DeliveryAddress2", _Basket.DeliveryAddress.Line2)
        'helper.Add("DeliveryAddress3", _Basket.DeliveryAddress.Line3)
        'helper.Add("DeliveryCity", _Basket.DeliveryAddress.Line4)
        'helper.Add("DeliveryPostCode", _Basket.DeliveryAddress.PostCode)
        'helper.Add("DeliveryCountry", _Basket.DeliveryAddress.Country.Code)
		
        'If _Basket.DeliveryAddress.Country.HasStates Then
        '    helper.Add("DeliveryState", _Basket.DeliveryAddress.State.Code)
        'End If

        'helper.Add("CustomerName", _Basket.Customer.Name.FullName)
		
        'helper.DoPost()
        
        'HandleResponse(helper, transaction, paymentSuccessCallback)
    End Sub
    
    Protected Sub HandleResponse(ByVal helper As SagePayTransactionHelper, ByVal transaction As SagePayDirectTransaction, ByVal paymentSuccessCallback As Action(Of ITransaction))
        ...
        '    'Else
        '    '    errors.Text = "<div class=""error sheen"" style=""white-space:normal !important;"">" & errorMessage & "</div><br />"
        '    'End If
        'End If
    End Sub
    ...
    Sub Handle3DSecureCallback()
        ...
        'Dim transaction As New SagePayDirectTransaction()
        'transaction.OrderID = _Helper.ResponseParams("order_id")

        'HandleResponse(_Helper, transaction, AddressOf GetParentControl.HandlePaymentSuccess)
    End Sub
    
    Sub DoRefund(originalTransaction As ITransaction, amount As Decimal, description As String, ByVal paymentSuccessCallback As Action(Of ITransaction))
        'Dim transaction As New SagePayDirectTransaction()
        ...
        'HandleResponse(_Helper, transaction, paymentSuccessCallback)
    End Sub
	...
	<script type="text/javascript">
		$(function(){
			...
			$('#card_types input').click(function(){
				...
				if(self.attr('has-start-date').toLowerCase() == 'true'){
					$('#start_date').stop().fadeTo(500, 1, function(){ $('#start_date select').css('text-decoration', 'none').attr('disabled', false); });
				}else{
					$('#start_date').stop().fadeTo(500, 0.5, function(){ $('#start_date select').css('text-decoration', 'line-through').attr('disabled', true); });
				}
	...
	<div id="payment_form">
		...
		<div class="payment-section"><label>Start Date</label><dd:droplist_month_picker id="start_date" year_offset="-10" runat="server" /></div>
